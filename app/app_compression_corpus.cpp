...
namespace simnet::app
{
    struct CompressionCorpusWriter::Impl
    {
            ...
            : config(std::move(writer_config))
        {
            ...
            if (exists) {
                ...
                auto const created = std::filesystem::create_directories(
    ...
    )
    {
        ...
        if (peer == 0U || encoded.kind != EncodeResultKind::Update || encoded.update.sequence == 0U
            || encoded.update.sequence != encoded.report.sequence || encoded.update.bytes.empty()
