...
export namespace simnet::app
{
    ...
    [[nodiscard]] std::string_view next_option_value(
        int& index,
        int argc,
        char** argv,
    ...
    [[nodiscard]] std::chrono::milliseconds milliseconds_option(
        int& index,
        int argc,
        char** argv,
        std::string_view option
    ...
    [[nodiscard]] PipelineDefinition make_snapshot_pipeline(
...
namespace simnet::app
{
    ...
    std::chrono::milliseconds milliseconds_option(
        int& index,
        int argc,
        char** argv,
        std::string_view option
    ...
    {
        auto const value = parse_unsigned<std::uint64_t>(next_option_value(index, argc, argv, option), option);
    ...
    PipelineDefinition make_snapshot_pipeline(
        SharedConfig const& shared,
        TransportConfig const& transport
    )
    {
        ...
        if (shared.pipeline.enable_quantization) {
            ...
            pipeline.quantization.position_bounds = make_centered_bounds(shared.simulation.world_half);
        }
