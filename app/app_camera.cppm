...
export namespace simnet::app
{
    ...
    [[nodiscard]] LockedChaseCameraPose locked_chase_camera_pose(
        Vec3f player_position,
        Vec3f player_heading
    ) noexcept;
}
...
{
    LockedChaseCameraPose locked_chase_camera_pose(
        Vec3f player_position,
        Vec3f player_heading
    ) noexcept
    {
        ...
        auto forward = is_finite(player_heading)
            ? normalize_or(player_heading, Vec3f { .z = 1.0F })
            : Vec3f { .z = 1.0F };

        auto constexpr maximum_pitch_radians = 1.483529864F;
        auto const maximum_vertical = std::sin(maximum_pitch_radians);
        auto const vertical = std::clamp(
            forward.y,
            -maximum_vertical,
            maximum_vertical
        );
        auto const horizontal = normalize_or(
            Vec3f { .x = forward.x, .z = forward.z },
            Vec3f { .z = 1.0F }
        );
        auto const horizontal_scale = std::sqrt(
            std::max(0.0F, 1.0F - vertical * vertical)
        );
        ...
        auto const right = normalize_or(
            cross(Vec3f { .y = 1.0F }, horizontal),
            Vec3f { .x = 1.0F }
        );
        auto const up = normalize_or(
            cross(forward, right),
            Vec3f { .y = 1.0F }
        );
        return {
