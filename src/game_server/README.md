...
`EntityKind` distinguishes Boids from the one supported session-owned Player without changing the snapshot record. The Player is included in extraction but has no `FlockRow`, velocity SoA row, or neighbour-rule work. Its latest semantic input is replaced atomically on the owner thread, then a single-threaded Flecs system performs bounded yaw/pitch steering and smooth speed targeting. Session disconnect deletes the Player through the indexed authoritative lifecycle.
