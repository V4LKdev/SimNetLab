...
`EntityKind` distinguishes `EntityKind::Boid` from the one supported session-owned `EntityKind::Player` without changing the snapshot record. The player is included in extraction but has no `FlockRow`, velocity SoA row, or neighbor-rule work. Its latest semantic input is replaced atomically on the owner thread, then a single-threaded Flecs system accelerates private yaw/pitch velocities, damps released axes, enforces angular-rate and pitch limits, targets speed smoothly, and integrates the authoritative pose. Paused worlds do not advance or accumulate player motion. Session disconnect deletes the player through the indexed authoritative lifecycle.
