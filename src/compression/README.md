...
The 17-byte network-order envelope contains `SNCZ`, protocol and schema versions, the Raw or Zstd
encoding, the declared uncompressed size, and the encoded payload size. Decoding requires one
complete frame with an exact known content size. Truncated frames, concatenated frames, trailing
bytes, corrupt data, and size mismatches are rejected. Caller output changes only after successful
