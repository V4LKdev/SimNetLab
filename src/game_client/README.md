...
It registers shared game components plus private client replication state. Call `register_client_game` once during client setup. `apply_client_snapshot_patch` consumes decoded `SnapshotUpdate` data and updates a Flecs client world. It maps classifications 1 and 2 back to Boid and Player component values. Unsupported nonzero classifications and updates older than the latest accepted client tick are rejected before mutating entities or client replication state. Equal ticks are accepted for future multi-packet ticks. Entity kind never depends on local-player ownership.
