...
void fit_popover(Rectangle &popover, Options const &options) noexcept {
  ...
  popover.height = 62.0F + static_cast<float>(visible_count) * 30.0F;
}

Rectangle inspector_tab(std::size_t index) noexcept {
  return {16.0F + static_cast<float>(index) * 104.0F, 64.0F, 96.0F, 28.0F};
}

void draw_text(Font font, char const *value, Vector2 position, float size,
               Color color) {
  DrawTextEx(font, value, position, size, 0.5F, color);
}
} // namespace
...
void Viewer::Impl::update_panel_input(RenderFrame const &frame,
  ...
  if (mouse.x < static_cast<float>(scene_rect_.x)) {
    ...
    if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
      for (std::size_t index = 0; index < 3U; ++index) {
        if (CheckCollisionPointRec(mouse, inspector_tab(index))) {
          ui_.page = static_cast<InspectorPage>(index);
        }
      }
    }
    auto const wheel = GetMouseWheelMove();
    if (wheel != 0.0F && mouse.y >= 104.0F &&
        mouse.y < static_cast<float>(config_.window_height) - 42.0F) {
  ...
  if (CheckCollisionPointRec(mouse, layout.toolbar_buttons[0]) &&
    ...
    auto row_y = layout.popover.y + 50.0F;
    for (auto &option : options) {
      if (!option.visible) {
        continue;
      }
      auto const rect = Rectangle{layout.popover.x + 12.0F, row_y,
                                  layout.popover.width - 24.0F, 28.0F};
      if (CheckCollisionPointRec(mouse, rect)) {
...
void Viewer::Impl::draw_panel(RenderFrame const &frame, bool valid_entities,
  ...
  draw_text(font_, config_.title.c_str(), {16.0F, 14.0F}, 18.0F,
            palette.primary);
  char role[96]{};
  ...
  draw_text(font_, role, {16.0F, 39.0F}, 12.0F, palette.secondary);

  constexpr std::array page_names{"Overview", "Network", "Entity"};
  for (std::size_t index = 0; index < page_names.size(); ++index) {
    auto const rect = inspector_tab(index);
    auto const active = page_index(ui_.page) == index;
    if (active) {
      DrawRectangleRec(rect, palette.raised);
      DrawRectangle(static_cast<int>(rect.x), static_cast<int>(rect.y + 26.0F),
                    static_cast<int>(rect.width), 2, palette.accent);
    }
    draw_text(font_, page_names[index], {rect.x + 6.0F, rect.y + 6.0F}, 13.0F,
              active ? palette.primary : palette.muted);
  }

  auto const content_top = 104;
  auto const content_bottom = static_cast<int>(config_.window_height) - 42;
  auto const content_height = content_bottom - content_top;
  auto &scroll = ui_.page_scroll[page_index(ui_.page)];
  auto content_y = static_cast<float>(content_top) - scroll;
  auto constexpr section_height = 34.0F;
  auto constexpr row_height = 23.0F;
  ...
  for (std::size_t section_index = 0;
    ...
    content_y += 10.0F;
    draw_text(font_, section.title.data(), {16.0F, content_y}, 13.0F,
              palette.accent);
    content_y += 24.0F;
    for (std::size_t row_index = section.first_row;
      ...
      if (row.full_width) {
        draw_text(font_, row.value.data(), {16.0F, content_y}, 12.0F,
                  value_color(row.state));
      } else {
        draw_text(font_, row.label.data(), {16.0F, content_y}, 12.0F,
                  palette.secondary);
        auto const value_width =
            MeasureTextEx(font_, row.value.data(), 12.0F, 0.5F).x;
        draw_text(font_, row.value.data(),
                  {std::max(154.0F, width - 16.0F - value_width), content_y},
                  12.0F, value_color(row.state));
      }
  ...
  draw_text(font_, footer, {16.0F, static_cast<float>(content_bottom + 13)},
            11.0F, palette.secondary);
}
...
void Viewer::Impl::draw_viewport_ui(RenderFrame const &frame,
  ...
  for (std::size_t index = 0; index < labels.size(); ++index) {
    ...
    draw_text(font_, labels[index], {rect.x + 11.0F, rect.y + 8.0F}, 12.0F,
  ...
  std::snprintf(mode, sizeof(mode), "Camera  %s", name);
  draw_text(font_, mode, {static_cast<float>(scene_rect_.x + 18), 19.0F}, 12.0F,
            palette.secondary);
  ...
  if (ui_.overlay_menu_open) {
    ...
    draw_text(font_, "VISUAL OVERLAYS",
              {layout.popover.x + 14.0F, layout.popover.y + 16.0F}, 13.0F,
              palette.accent);
    ...
    for (auto const &option : options) {
      ...
      DrawRectangleLinesEx({row.x + 4.0F, row.y + 6.0F, 14.0F, 14.0F}, 1.0F,
                           *option.value ? palette.accent : palette.border);
      if (*option.value) {
        DrawRectangle(static_cast<int>(row.x + 7.0F),
                      static_cast<int>(row.y + 9.0F), 8, 8, palette.accent);
      }
      draw_text(font_, option.label, {row.x + 28.0F, row.y + 7.0F}, 12.0F,
                palette.primary);
      y += 30.0F;
    }
  }
  ...
  if (selected_entity_frame_.has_value() && ui_.page != InspectorPage::Entity) {
    auto const card = Rectangle{
        static_cast<float>(scene_rect_.x) +
            (static_cast<float>(scene_rect_.width) - 420.0F) * 0.5F,
        static_cast<float>(scene_rect_.height) - 68.0F, 420.0F, 48.0F};
    ...
    draw_text(font_, value, {card.x + 16.0F, card.y + 16.0F}, 13.0F,
              palette.selection);
  }
}
...
void Viewer::Impl::draw_help_overlay(RenderFrame const &frame) const {
  draw_text(font_, "F12  Help",
            {static_cast<float>(scene_rect_.x + scene_rect_.width - 86),
             static_cast<float>(scene_rect_.height - 28)},
            11.0F, palette.muted);
  if (!ui_.help_open) {
    return;
  }
  auto const rect =
      Rectangle{static_cast<float>(scene_rect_.x + 32), 62.0F, 430.0F,
                frame.game_camera.has_value() ? 360.0F : 304.0F};
  DrawRectangleRec(rect, palette.raised);
  DrawRectangleLinesEx(rect, 1.0F, palette.border);
  auto y = rect.y + 18.0F;
  auto line = [&](char const *value, Color color = palette.secondary) {
    draw_text(font_, value, {rect.x + 18.0F, y}, 12.0F, color);
    y += 22.0F;
  };
