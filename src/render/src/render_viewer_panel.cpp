...
namespace simnet
{
    ...
    void Viewer::Impl::update_panel_input(
    ...
    )
    {
        ...
        DrawRectangle(0, 0, scene_rect_.x, static_cast<int>(config_.window_height), palette.panel);
        DrawLine(
            scene_rect_.x - 1,
            0,
            scene_rect_.x - 1,
        ...
        draw_text(font_, context, {16.0F, 46.0F}, typography.context, palette.secondary);
        ...
        for (std::size_t section_index = 0; section_index < panel_model_.section_count;
            ...
            DrawLine(
                16,
                static_cast<int>(content_y + 3.0F),
                scene_rect_.x - 16,
                static_cast<int>(content_y + 3.0F),
                palette.divider
            );
        ...
        if (max_scroll > 0.0F) {
            ...
            auto const thumb_height = std::max(28.0F, track.height * track.height / total_height);
            auto const thumb_y = track.y + (track.height - thumb_height) * scroll / max_scroll;
            DrawRectangleRec({track.x, thumb_y, track.width, thumb_height}, palette.accent);
        }
