...
namespace simnet
{
    ...
    void Viewer::Impl::prepare_instances(RenderEntityView const& entities, RenderStats& stats)
    {
        clear_instances();
        {
            SIMNET_TRACE_SCOPE_CATEGORY(
                "render.prepare.capacity_growth",
    ...
    void Viewer::Impl::draw_stationary_observer(
        StationaryObserverView const& stationary_observer,
        RenderStats& stats
    )
    {
        SIMNET_TRACE_SCOPE_CATEGORY(
    ...
    void Viewer::Impl::draw_debug_labels(DebugPrimitiveView const& debug) const
    {
        ...
        auto label = [&](Vec3f position, std::string_view value, DebugColor color) {
            ...
            char text[64]{};
            std::snprintf(text, sizeof(text), "%.*s", static_cast<int>(value.size()), value.data());
            DrawTextEx(
                font_,
                text,
                {screen.x + 6.0F, screen.y - 8.0F},
                typography.debug_label,
                0.75F,
                to_raylib(color)
            );
            ++drawn;
