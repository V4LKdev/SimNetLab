...
namespace simnet
{
    namespace
    {
        ...
        void add_text(
            ...
            UiValueState state = UiValueState::Muted
        ) noexcept
        {
            if (model.row_count >= model.rows.size() || model.section_count == 0U) {
                return;
    ...
    void Viewer::Impl::build_panel_model(RenderFrame const& frame, bool valid_entities)
    {
        ...
        if (ui_.page == Overview) {
            ...
            if (frame.info.simulation_paused.has_value()) {
                add_row(
                    panel_model_,
                    "State",
                    *frame.info.simulation_paused ? Warning : Success,
                    "%s",
                    *frame.info.simulation_paused ? "Paused" : "Running"
                );
            } else if (
                frame.info.context.kind == ViewerKind::Client && frame.info.connection.has_value()
            ) {
                add_row(
                    panel_model_,
                    "State",
            ...
            if (valid_entities) {
                ...
                add_row(
                    ...
                    frame.entities.hues.size()
                );
            }
            auto const extent = frame.info.world_bounds.max - frame.info.world_bounds.min;
            add_row(
                panel_model_,
                "World size",
                Muted,
            ...
            if (frame.info.interpolation.has_value()) {
                ...
                add_row(
                    panel_model_,
                    "Interpolation",
                    value.interpolating ? Success : Muted,
                    "%s",
                    !value.enabled            ? "Off"
            ...
            if (frame.spatial.has_value()) {
                ...
                add_row(panel_model_, "Occupied cells", Normal, "%u", value.occupied_cell_count);
                add_row(panel_model_, "Maximum occupancy", Normal, "%u", value.max_cell_occupancy);
                add_row(
                    panel_model_,
                    "Average occupancy",
                    Normal,
            ...
            if (valid_entities && completed_stats_.instance_count != frame.entities.size()) {
                add_row(
                    panel_model_,
                    "Rendered",
                    Warning,
            ...
            add_row(panel_model_, "Draw calls", Normal, "%u", completed_stats_.draw_calls);
            return;
        }
        ...
        if (ui_.page == Network) {
            ...
            if (!frame.info.replication.has_value()) {
                ...
                add_text(panel_model_, "Snapshot state appears after a session is established.");
                return;
            }

            auto const& value = *frame.info.replication;
            add_section(panel_model_, "REPLICATION");
            auto sequence = [&](std::string_view label, std::optional<SequenceId> current) {
            ...
            if (value.retained_snapshot_count.has_value()) {
                ...
                auto const expanded
                    = (ui_.expanded_sections[static_cast<std::size_t>(Network)] & (1U << index))
                    != 0U;
        ...
        if (ui_.page == Setup) {
            ...
            if (setup_revision_ != frame.setup->revision) {
                ...
                for (auto const& source : frame.setup->sections) {
                    add_section(panel_model_, source.title, true, source.initially_expanded);
            ...
            for (std::size_t index = 0; index < panel_model_.section_count; ++index) {
                auto const expanded
                    = (ui_.expanded_sections[static_cast<std::size_t>(Setup)] & (1U << index))
                    != 0U;
        ...
        auto const* details
            = frame.selected_details.has_value() && frame.selected_details->id == selected.id
        ...
        if (details->raw_candidate_count.has_value()) {
            add_row(panel_model_, "Candidates", Normal, "%u", *details->raw_candidate_count);
        }
        if (details->retained_neighbor_count.has_value()) {
            add_row(panel_model_, "Retained", Normal, "%u", *details->retained_neighbor_count);
        }
        ...
        auto contribution = [&](std::string_view label, std::optional<Vec3f> current) {
            if (current.has_value()) {
        ...
        auto const advanced_expanded
            = (ui_.expanded_sections[static_cast<std::size_t>(Entity)] & (1U << advanced_index))
            != 0U;
        ...
        if (details->last_update_sequence.has_value()) {
            add_row(panel_model_, "Update sequence", Normal, "%u", *details->last_update_sequence);
        }
        ...
        if (details->applied_hue_step.has_value()) {
            add_row(panel_model_, "Hue step", Normal, "%.4f", *details->applied_hue_step);
        }
