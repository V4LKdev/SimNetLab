...
void validate_config(simnet::ViewerConfig const &config) {
  ...
  if (config.panel_width >= config.window_width) {
    throw std::runtime_error(
        "viewer panel_width must be less than window_width");
  }
  ...
  if (config.entity_scale <= 0.0F || config.picking_radius <= 0.0F) {
    throw std::runtime_error(
        "viewer entity_scale and picking_radius must be positive");
  }
...
normalized_or_forward(simnet::Vec3f heading) noexcept {
...
[[nodiscard]] simnet::Vec3f cross(simnet::Vec3f lhs,
                                  simnet::Vec3f rhs) noexcept {
  return {
...
[[nodiscard]] WorldUpBasis world_up_basis(simnet::Vec3f forward) noexcept {
  auto const horizontal = simnet::normalize_or(
      simnet::Vec3f{.x = forward.x, .z = forward.z}, simnet::Vec3f{.z = 1.0F});
  auto const right = simnet::normalize_or(
      cross(simnet::Vec3f{.y = 1.0F}, horizontal), simnet::Vec3f{.x = 1.0F});
  return {
      .right = right,
      .up =
          simnet::normalize_or(cross(forward, right), simnet::Vec3f{.y = 1.0F}),
  };
}
...
[[nodiscard]] Mesh make_directional_mesh() {
  ...
  mesh.indices =
      static_cast<unsigned short *>(MemAlloc(sizeof(unsigned short) * 12));
  if (mesh.vertices == nullptr || mesh.normals == nullptr ||
      mesh.texcoords == nullptr || mesh.indices == nullptr) {
    throw std::runtime_error("failed to allocate directional entity mesh");
  }

  float const vertices[] = {
      0.0F,  0.0F,   1.25F,  -0.45F, -0.25F, -0.65F,
      0.45F, -0.25F, -0.65F, 0.0F,   0.45F,  -0.35F,
  };
  float const normals[] = {
      0.0F, 0.0F,  1.0F,  -0.5F, -0.3F, -0.8F,
      0.5F, -0.3F, -0.8F, 0.0F,  0.8F,  -0.5F,
  };
...
void bake_mesh_correction(Mesh &mesh, Matrix correction) {
  ...
  for (auto index = 0; index < mesh.vertexCount; ++index) {
    ...
    auto const corrected =
        Vector3Transform({mesh.vertices[offset], mesh.vertices[offset + 1],
  ...
  for (auto index = 0; index < mesh.vertexCount; ++index) {
    ...
    auto const normal = Vector3{
        correction.m0 * mesh.normals[offset] +
            correction.m4 * mesh.normals[offset + 1] +
            correction.m8 * mesh.normals[offset + 2],
        correction.m1 * mesh.normals[offset] +
            correction.m5 * mesh.normals[offset + 1] +
            correction.m9 * mesh.normals[offset + 2],
        correction.m2 * mesh.normals[offset] +
            correction.m6 * mesh.normals[offset + 1] +
...
Viewer::Impl::Impl(ViewerConfig config)
  ...
  InitWindow(static_cast<int>(config_.window_width),
             static_cast<int>(config_.window_height), config_.title.c_str());
  if (!IsWindowReady()) {
  ...
  load_entity_model();
  shader_ = LoadShaderFromMemory(instancing_vertex_shader,
                                 instancing_fragment_shader);
  instancing_available_ =
      shader_.id != 0 && model_.meshCount > 0 && model_.materialCount > 0 &&
  ...
  if (instancing_available_) {
    ...
    TraceLog(LOG_WARNING,
             "SimNet viewer instanced entity drawing is unavailable");
  }
...
[[nodiscard]] ViewerResult Viewer::Impl::draw(RenderFrame const &frame) {
  ...
  {
    SIMNET_TRACE_SCOPE_CATEGORY("render.instance_preparation",
                                simnet::LogCategory::Render);
    if (valid_entities) {
  ...
  {
    SIMNET_TRACE_SCOPE_CATEGORY("render.scene_submission",
                                simnet::LogCategory::Render);
  ...
  DrawTextureRec(scene_.texture,
                 Rectangle{0.0F, 0.0F, static_cast<float>(scene_rect_.width),
  ...
  SIMNET_TRACE_PLOT("render.active_hue_buckets",
                    static_cast<double>(stats.active_hue_buckets));
  SIMNET_TRACE_PLOT("render.selected_trail_points",
                    static_cast<double>(selected_trail_.size()));
  if (frame.spatial.has_value()) {
    SIMNET_TRACE_PLOT("render.displayed_spatial_cells",
...
void Viewer::Impl::load_entity_model() {
  if (!config_.entity_mesh_path.empty()) {
    ...
    if (model_.meshCount > 0 && model_.materialCount > 0 &&
        model_.meshes != nullptr && model_.materials != nullptr &&
        model_.meshMaterial != nullptr) {
    ...
    TraceLog(LOG_WARNING,
             "SimNet viewer mesh load failed, using procedural wedge");
  }
...
Viewer::Impl::find_selected_entity(RenderEntityView const &entities) const {
  ...
  for (std::size_t index = 0; index < entities.size(); ++index) {
    if (entities.ids[index] == *selected_entity_ &&
        finite(entities.positions[index]) && finite(entities.headings[index])) {
...
void Viewer::Impl::clear_selection(ViewerResult &result,
                                   bool preserve_navigation_anchor) {
...
void Viewer::Impl::update_camera_position(float yaw, float pitch,
...
void Viewer::Impl::reset_overview_camera(Vec3f center) noexcept {
  ...
  overview_distance_ =
      std::clamp(max_distance_ * 0.45F, min_distance_, max_distance_);
}
...
void Viewer::Impl::reset_detail_camera(float world_extent) noexcept {
  ...
  detail_distance_ =
      std::clamp(std::max(config_.entity_scale * 10.0F, world_extent * 0.03F),
                 detail_min_distance_, detail_max_distance_);
}
...
Viewer::Impl::ray_sphere_hit_distance(Ray ray, Vector3 center,
...
void Viewer::Impl::update_selection(RenderEntityView const &entities,
  ...
  auto const ray = GetScreenToWorldRayEx(scene_mouse, camera_,
                                         scene_rect_.width, scene_rect_.height);
  ...
  for (std::size_t index = 0; index < entities.size(); ++index) {
    ...
    auto const distance = ray_sphere_hit_distance(ray, to_raylib(position),
                                                  config_.picking_radius);
...
void Viewer::Impl::select_entity(SelectedEntity selected,
                                 ViewerResult &result) {
...
void Viewer::Impl::select_adjacent_entity(RenderEntityView const &entities,
                                          int direction, ViewerResult &result) {
  if (!entities.valid()) {
    return;
  }
  auto const anchor =
      selected_entity_.has_value() ? selected_entity_ : navigation_anchor_;
