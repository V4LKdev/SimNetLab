...
namespace simnet
{
    ...
    void Viewer::Impl::draw_viewport_ui(RenderFrame const& frame, ViewerResult const&)
    {
        ...
        draw_text(
            font_,
            mode,
            {backing.x + 10.0F, backing.y + 9.0F},
            typography.toolbar,
            palette.primary
        );
        ...
        if (ui_.popover == OpenPopover::Camera)
        {
            ...
            for (auto const& option : options)
            {
                ...
                draw_text(
                    font_,
                    label,
                    {layout.popover.x + 18.0F, y + 7.0F},
                    typography.body,
                    active ? palette.accent : palette.primary
                );
                y += overlay_row_height;
            }
            DrawLine(
                static_cast<int>(layout.popover.x + 14.0F),
            ...
            draw_text(
                font_,
                "\uf2f9  Reset active camera",
                {layout.popover.x + 18.0F, y + 15.0F},
                typography.body,
                palette.primary
            );
        }
        else if (ui_.popover == OpenPopover::Overlays)
        {
            ...
            for (auto const& option : options)
            {
                ...
                DrawRectangleLinesEx(
                    {row.x + 4.0F, row.y + 8.0F, 16.0F, 16.0F},
                    1.0F,
        ...
        if (selected_entity_frame_.has_value() && ui_.page != InspectorPage::Entity)
        {
            ...
            draw_text(
                font_,
                value,
                {card.x + 16.0F, card.y + 17.0F},
                typography.context_card,
                palette.selection
            );
        }
    }
    ...
    {
        auto const forward = normalize_or(
            Vec3f{
                .x = camera_.target.x - camera_.position.x,
        ...
        auto const origin = Vector2{
            static_cast<float>(scene_rect_.x + 58),
        ...
        auto axis = [&](Vec3f world, Color color, char const* label)
        {
            ...
            draw_text(
                font_,
                label,
                {endpoint.x + 3.0F, endpoint.y - 8.0F},
                typography.secondary,
                color
            );
        };
