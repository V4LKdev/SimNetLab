...
The viewer uses 32 persistent hue buckets and one instanced draw per non-empty bucket. It renders world bounds, optional axes, and an overview orbit camera. Right drag orbits, the wheel zooms, and `R` resets the active camera. The panel shows application, connection, replication, simulation, rendering, camera, and control facts supplied by the owning app. The Server viewer can pause locally. A ready Client viewer requests an authoritative pause state and continues applying snapshots while paused.

Left click in the scene viewport performs a nearest-hit ray-to-sphere selection using the configured picking radius. The Viewer stores the stable `EntityNetId`, not a frame-local array index. A hit enters Entity Detail mode with an independent orbit camera and a wire highlight. An empty scene click, Escape, or Return to overview clears the selection. If the selected ID is absent from a later valid frame, the Viewer also clears it and returns to Overview. Optional `SelectedEntityDetails` are shown only when their ID matches the current selection.
