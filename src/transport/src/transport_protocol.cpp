...
DisconnectCode identity_mismatch(SessionIdentity const &actual, SessionIdentity const &expected) noexcept {
...
std::uint8_t lane_index(Lane lane) noexcept { return static_cast<std::uint8_t>(lane); }

bool valid_lane(Lane lane) noexcept { return lane_index(lane) < channel_count; }
...
void write_u8(std::vector<Byte> &bytes, std::uint8_t value) { bytes.push_back(static_cast<Byte>(value)); }
...
bool read_u32(Byte const *data, std::size_t size, std::size_t &offset, std::uint32_t &value) {
  ...
  std::uint8_t c{};
  std::uint8_t d{};
  if (!read_u8(data, size, offset, a) || !read_u8(data, size, offset, b) || !read_u8(data, size, offset, c) ||
      !read_u8(data, size, offset, d)) {
    return false;
  }
...
bool decode_session_message(Byte const *data, std::size_t size, SessionMessage &message) {
  ...
  if (message.kind == SessionMessageKind::ClientHello) {
    return payload_size == 24U && read_u32(data, size, offset, message.identity.application_protocol_version) &&
  ...
  if (message.kind == SessionMessageKind::ServerReject) {
    ...
    return valid_disconnect_code(message.reject_code) && message.reject_code != DisconnectCode::None;
  }
