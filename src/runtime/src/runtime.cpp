...
namespace simnet
{
    ...
    ) noexcept
    {
        ...
        if (settings.max_ticks != 0U) {
            auto const remaining_ticks = clock.tick < settings.max_ticks
                ? settings.max_ticks - clock.tick
                : Tick {};
            auto const bounded_remaining = std::min<Tick>(
                remaining_ticks,
    ...
    ShutdownReason reached_runtime_limit(
        RuntimeSettings const& settings,
