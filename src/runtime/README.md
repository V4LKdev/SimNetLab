...
`plan_runtime_frame` clamps negative and oversized frame deltas, limits
catch-up work, and discards excess whole-step backlog after the step cap while
preserving the fractional accumulator remainder. Discarded time is reported in
both the returned `RuntimeFramePlan` and cumulative `RuntimeStats`.
