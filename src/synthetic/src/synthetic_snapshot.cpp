...
namespace
{
    ...
    {
        if (!std::isfinite(settings.entity_change_fraction) || settings.entity_change_fraction < 0.0
            || settings.entity_change_fraction > 1.0) {
    ...
    ) noexcept
    {
        return left.seed == right.seed && left.entity_count == right.entity_count
            && left.pattern == right.pattern && same_binary32(left.bounds.min.x, right.bounds.min.x)
