...
Config parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--entities") cfg.entities = parse_size(value(arg), arg);
        else if (arg == "--train-ticks") cfg.train_ticks = parse_size(value(arg), arg);
        else if (arg == "--test-ticks") cfg.test_ticks = parse_size(value(arg), arg);
        else if (arg == "--repeats") cfg.repeats = parse_size(value(arg), arg);
        else if (arg == "--mtu") cfg.mtu = parse_size(value(arg), arg);
        else if (arg == "--headroom") cfg.headroom = parse_size(value(arg), arg);
        else if (arg == "--dict-size") cfg.dict_size = parse_size(value(arg), arg);
        else if (arg == "--zstd-level") cfg.zstd_level = static_cast<int>(parse_size(value(arg), arg));
        else if (arg == "--world-half") cfg.world_half = static_cast<float>(parse_double(value(arg), arg));
        else if (arg == "--speed") cfg.speed = static_cast<float>(parse_double(value(arg), arg));
        else if (arg == "--dt") cfg.dt = static_cast<float>(parse_double(value(arg), arg));
        else if (arg == "--seed") cfg.seed = static_cast<std::uint32_t>(parse_size(value(arg), arg));
        else if (arg == "--loss") cfg.loss_rate = parse_double(value(arg), arg);
        else if (arg == "--csv") cfg.csv = value(arg);
        else if (arg == "--help" || arg == "-h") {
            std::cout
                << "Usage: network_probe [options]\n"
    ...
    if (cfg.mtu <= cfg.headroom) fail("MTU must be larger than headroom");
    if (cfg.world_half <= 0 || cfg.speed < 0 || cfg.dt <= 0) fail("invalid simulation settings");
    if (cfg.loss_rate < 0.0 || cfg.loss_rate >= 1.0) fail("loss must be in [0, 1)");
    return cfg;
}

Vec3 normalize(Vec3 v) {
    const float length_sq = v.x * v.x + v.y * v.y + v.z * v.z;
    if (length_sq <= 1e-12F) return {1.0F, 0.0F, 0.0F};
...
class Simulation {
public:
    explicit Simulation(const Config& cfg) : cfg_(cfg), rng_(cfg.seed) {
...
EncodedSnapshot encode_full_quantized(const std::vector<QuantizedBoid>& boids) {
    ...
    for (const QuantizedBoid& b : boids) {
        append_le(out.bytes, b.id);
        for (const auto p : b.position) append_le(out.bytes, p);
        for (const auto h : b.heading) append_le(out.bytes, h);
        out.bytes.push_back(b.hue);
        out.record_offsets.push_back(out.bytes.size());
    }
    return out;
...
EncodedSnapshot encode_delta_quantized(
    ...
    if (current.size() != baseline.size()) fail("delta baseline size mismatch");
    ...
    for (std::size_t i = 0; i < current.size(); ++i) {
        ...
        if (now.id != old.id) fail("delta baseline ID mismatch");

        append_le(out.bytes, now.id);
        std::uint8_t mask = 0;
        if (now.position != old.position) mask |= 0x01u;
        if (now.heading != old.heading) mask |= 0x02u;
        if (now.hue != old.hue) mask |= 0x04u;
        ...
        if ((mask & 0x04u) != 0) out.bytes.push_back(now.hue);
        out.record_offsets.push_back(out.bytes.size());
    }
    return out;
}
...
Bytes train_dictionary(std::size_t capacity, const std::vector<Bytes>& samples) {
    std::size_t total = 0;
    for (const auto& sample : samples) total += sample.size();
    Bytes joined;
    ...
    const std::size_t result = ZDICT_trainFromBuffer(
        dictionary.data(), dictionary.size(), joined.data(), sizes.data(), static_cast<unsigned>(sizes.size()));
...
public:
    ZstdCodec(int level, Bytes dictionary = {})
        : level_(level), dictionary_(std::move(dictionary)),
          cctx_(ZSTD_createCCtx()), dctx_(ZSTD_createDCtx()) {
        if (!cctx_ || !dctx_) fail("failed to create Zstd contexts");
        if (!dictionary_.empty()) {
            ...
            if (!cdict_ || !ddict_) fail("failed to create Zstd dictionary contexts");
        }
    }
    ...
    Bytes compress(std::span<const std::uint8_t> input) const {
        ...
        if (cdict_) {
            result = ZSTD_compress_usingCDict(
                cctx_, output.data(), output.size(), input.data(), input.size(), cdict_);
        } else {
            result = ZSTD_compressCCtx(
                cctx_, output.data(), output.size(), input.data(), input.size(), level_);
        }
    ...
    Bytes decompress(std::span<const std::uint8_t> input, std::size_t expected_size) const {
        ...
        if (ddict_) {
            result = ZSTD_decompress_usingDDict(
                dctx_, output.data(), output.size(), input.data(), input.size(), ddict_);
        } else {
            result = ZSTD_decompressDCtx(
                dctx_, output.data(), output.size(), input.data(), input.size());
        }
        if (ZSTD_isError(result)) {
            fail(std::string("Zstd decompression failed: ") + ZSTD_getErrorName(result));
        }
        if (result != expected_size) fail("Zstd decompressed size mismatch");
        return output;
    }
...
std::vector<PacketSlice> packetize_raw(const EncodedSnapshot& snapshot, std::size_t payload) {
    ...
    while (begin_record < records) {
        ...
        while (end_record < records) {
            ...
            if (bytes > payload) break;
            ++end_record;
        }
        if (end_record == begin_record) fail("one encoded entity exceeds packet payload");
...
bool compressed_range_fits(
    ...
    const auto end = snapshot.record_offsets[end_record];
    return codec.compress(std::span(snapshot.bytes).subspan(begin, end - begin)).size() <= payload;
}
...
Measurement measure_none(
    ...
    const auto [min_it, max_it] = std::minmax_element(
        packets.begin(), packets.end(), [](const auto& a, const auto& b) { return a.entities < b.entities; });
    const double complete = std::pow(1.0 - loss_rate, static_cast<double>(packets.size()));
    return {
        snapshot.name, "none", "per_packet", entities,
        snapshot.bytes.size(), snapshot.bytes.size(), packets.size(), 1.0,
        static_cast<double>(entities) / static_cast<double>(packets.size()),
        min_it->entities, max_it->entities, 0.0, 0.0,
        complete, 1.0 - loss_rate
    };
}
...
Measurement measure_whole(
    ...
    for (std::size_t i = 0; i < repeats; ++i) {
        ...
        if (decoded != snapshot.bytes) fail("whole-frame roundtrip mismatch");
    }
    ...
    return {
        snapshot.name, std::move(compression_name), "whole_snapshot", entities,
        snapshot.bytes.size(), compressed.size(), packets,
...
Measurement measure_per_packet(
    ...
    for (std::size_t repeat = 0; repeat < repeats; ++repeat) {
        ...
        for (const PacketSlice& packet : packets) {
            compressed_packets.push_back(codec.compress(
                std::span(snapshot.bytes).subspan(packet.offset, packet.size)));
        }
        compress_time += micros(Clock::now() - start);
    }
    ...
    for (std::size_t repeat = 0; repeat < repeats; ++repeat) {
        ...
        for (std::size_t i = 0; i < packets.size(); ++i) {
            ...
            const auto expected = std::span(snapshot.bytes).subspan(packets[i].offset, packets[i].size);
    ...
    for (const auto& packet : compressed_packets) payload_bytes += packet.size();
    const auto [min_it, max_it] = std::minmax_element(
        packets.begin(), packets.end(), [](const auto& a, const auto& b) { return a.entities < b.entities; });
    const double complete = std::pow(1.0 - loss_rate, static_cast<double>(packets.size()));

    return {
        snapshot.name, std::move(compression_name), "per_packet", entities,
        snapshot.bytes.size(), payload_bytes, packets.size(),
        static_cast<double>(payload_bytes) / static_cast<double>(snapshot.bytes.size()),
        static_cast<double>(entities) / static_cast<double>(packets.size()),
        min_it->entities, max_it->entities,
        compress_time / static_cast<double>(repeats),
        decompress_time / static_cast<double>(repeats),
        complete, 1.0 - loss_rate
    };
...
struct Accumulator {
    ...
    void add(const Measurement& m) {
        ...
        if (m.min_entities_per_packet != 0) {
            min_entities_per_packet = std::min(min_entities_per_packet, static_cast<double>(m.min_entities_per_packet));
        }
        max_entities_per_packet = std::max(max_entities_per_packet, static_cast<double>(m.max_entities_per_packet));
    ...
    Measurement mean() const {
        ...
        return {
            ...
            std::isinf(min_entities_per_packet) ? 0 : static_cast<std::size_t>(min_entities_per_packet),
...
std::vector<Bytes> make_training_samples(
    const Config& cfg,
    const std::string& encoding_name) {
    Config training_cfg = cfg;
    training_cfg.seed ^= 0xA5A5'9E37u; // keep training trajectories separate from benchmark data
    ...
    for (std::size_t tick = 0; tick < cfg.train_ticks; ++tick) {
        ...
        if (encoding_name == "full_f32") snapshot = encode_full_f32(sim.boids());
        else if (encoding_name == "full_quantized") snapshot = encode_full_quantized(current);
        else snapshot = encode_delta_quantized(current, previous);
        ...
        for (const PacketSlice& packet : raw_packets) {
            samples.emplace_back(
                ...
                snapshot.bytes.begin() + static_cast<std::ptrdiff_t>(packet.offset + packet.size));
        }
        previous = current;
    }
    return samples;
...
void print_table(const std::vector<Measurement>& rows, const Config& cfg) {
    ...
    std::cout
        ...
        << std::setw(9) << "ratio"
        << std::setw(8) << "pkts"
        << std::setw(11) << "boids/pkt"
        << std::setw(12) << "comp us"
        << std::setw(12) << "decomp us"
    ...
    for (const auto& row : rows) {
        std::cout
            << std::left
            << std::setw(18) << row.encoding
            << std::setw(12) << row.compression
            << std::setw(16) << row.scope
            << std::right
            << std::setw(10) << row.source_bytes
            << std::setw(10) << row.payload_bytes
            << std::setw(9) << std::fixed << std::setprecision(3) << row.ratio
            << std::setw(8) << row.packets
            << std::setw(11) << std::setprecision(1) << row.entities_per_packet
            << std::setw(12) << std::setprecision(1) << row.compress_us
            << std::setw(12) << row.decompress_us
            << std::setw(12) << std::setprecision(2) << row.complete_probability * 100.0
            << std::setw(12) << row.expected_usable_fraction * 100.0
            << '\n';
    }
...
void write_csv(const std::filesystem::path& path, const std::vector<Measurement>& rows) {
    ...
    if (!out) fail("failed to open CSV output");
    ...
    for (const auto& row : rows) {
        out << row.encoding << ',' << row.compression << ',' << row.scope << ','
            << row.entities << ',' << row.source_bytes << ',' << row.payload_bytes << ','
            << row.ratio << ',' << row.packets << ',' << row.entities_per_packet << ','
            << row.min_entities_per_packet << ',' << row.max_entities_per_packet << ','
            << row.compress_us << ',' << row.decompress_us << ','
            << row.complete_probability << ',' << row.expected_usable_fraction << '\n';
    }
}
...
int main(int argc, char** argv) {
    try {
        ...
        for (const auto& encoding : encoding_names) {
            ...
            std::cout << "  trained dictionary: " << dictionaries.back().size()
                      << " bytes from " << samples.size() << " packet-like samples\n";
        }
        ...
        for (std::size_t tick = 0; tick < cfg.test_ticks; ++tick) {
            ...
            for (std::size_t e = 0; e < snapshots.size(); ++e) {
                ...
                accumulators[base + 0].add(measure_none(snapshot, cfg.entities, payload, cfg.loss_rate));
                accumulators[base + 1].add(measure_whole(
                    snapshot, cfg.entities, payload, cfg.loss_rate, "zstd", plain, cfg.repeats));
                accumulators[base + 2].add(measure_per_packet(
                    snapshot, cfg.entities, payload, cfg.loss_rate, "zstd", plain, cfg.repeats));
                accumulators[base + 3].add(measure_whole(
                    snapshot, cfg.entities, payload, cfg.loss_rate, "zstd_dict", dict, cfg.repeats));
        ...
        for (const auto& accumulator : accumulators) rows.push_back(accumulator.mean());
